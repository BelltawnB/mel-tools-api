// daily-summary worker — scheduled cron trigger
// Set cron in wrangler.toml: "0 7 * * *"
//
// Required env vars (Workers Secrets or wrangler.toml [vars]):
...
async function redis(env, command, ...args) {
  const url = `${env.UPSTASH_REDIS_REST_URL}/${[command, ...args].map(encodeURIComponent).join("/")}`;
...
export default {
  async scheduled(event, env, ctx) {
    ...
    const spend  = parseFloat((await redis(env, "GET", `spend:${day}`)) || "0");
    const logRaw = await redis(env, "LRANGE", `log:${day}`, "0", "-1") || [];
    ...
    await fetch("https://api.sendgrid.com/v3/mail/send", {
      ...
      body: JSON.stringify({
        personalizations: [{ to: [{ email: env.SUMMARY_EMAIL }] }],
        from: { email: env.SUMMARY_EMAIL, name: "MEL Tools" },
